...
import { 
    receber_data_valida,
    receber_um_texto_de_no_maximo_n_caracteres,
...
function main(){
    ...
    while (opcao > 0 || opcao > 16){
        ...
        if (opcao === 10){
            const tamanho_do_texto = Number(input('Informe a quantidade máxima de caracteres: '))
            let texto = input('Texto: ')

            receber_um_texto_de_no_maximo_n_caracteres('Informe um texto: ', tamanho_do_texto, texto)
        }
        /*if (opcao === 11){
            let tamanho_do_texto = Number(input('Informe a quantidade mínima de caracteres: '))
            let texto = input('Texto: ')
        ...
        if (opcao === 12){
            const tamanho_maximo_de_caracteres = Number(input('Tamanho do texto: ')) 
            
            receber_um_valor_texto_de_no_maximo_n_caracteres('Escreva um texto com quantidades de caracteres inferior: ', tamanho_maximo_de_caracteres)
        }

        if (opcao === 15){
            //const data = Number(input('Data: ').split('/'))
            const [dia, mes, ano] = input('Data: ').split('/').map(Number)

            receber_data_valida('Informe uma data válida: ', dia, mes, ano)
        }*/
